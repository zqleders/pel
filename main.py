...
def run():
    ...
    with sync_playwright() as p:
        ...
        # 检查指定站点状态，如果不是 502 Bad Gateway 则点击 RESTART 按钮
        print("正在检查 https://pella.doit.us.ci 运行状态...")
        try:
            check_res = requests.get("https://pella.doit.us.ci", timeout=10)
            status_text = check_res.text
        except Exception as e:
            status_text = str(e)

        if "502 Bad Gateway" not in status_text:
            ...
            try:
                ...
                restart_btn.click()
                print("已成功点击 RESTART 按钮。")
                page.wait_for_timeout(5000)
            except Exception as e:
                print(f"点击 RESTART 按钮时出错: {e}")
        else:
